...
<div class="container text-center" style="margin-top: 7em;">
  <h2 class="fw-bold mb-4">Koleksi Poster Film Favoritmu 🖼️🎬</h2>
</div>
...
<div class="container">
  <div class="row row-cols-2 row-cols-md-4 g-4">
    ...
    // Ambil data dari tabel products dengan kategori 'poster_film'
    $result = $koneksi->query("SELECT * FROM products WHERE kategori = 'poster_film' ORDER BY id_product DESC");

    // Tampilkan setiap produk sebagai kartu
    while ($poster_film = $result->fetch_assoc()) {
      echo '
      <div class="col">
        <div class="card poster_film-card text-center p-2">
          <img src="admin/'.htmlspecialchars($poster_film["gambar"]).'" class="card-img-top poster_$poster_film-img" alt="'.htmlspecialchars($poster_film["nama_produk"]).'" />
          <div class="card-body">
            <p class="poster_film-title">'.$poster_film["nama_produk"].'</p>
            <p class="poster_film-price">Rp '.number_format($poster_film["harga"], 0, ',', '.').'</p>
...
<footer class="footer text-white py-4 shadow" style="background-color: yellow; color: black; margin-top: 5em;">
  <div class="container text-center">
    ...
    <p class="text-muted small mb-0">
      Lengkapi dinding kamar atau ruang kerja kamu dengan poster film keren dari Nonton Skuy! 🎥✨
    </p>
  </div>
</footer>
